...
import top.sxuan.exam.service.IAuthService;
import top.sxuan.exam.service.ILogService;
import top.sxuan.exam.utils.*;
import top.sxuan.exam.utils.security.SysUserDetails;
...
public class AuthServiceImpl implements IAuthService {
    ...
    public Result<String> login(HttpServletRequest request, LoginForm loginForm) {
        ...
        String token = jwtUtil.createJwt(userInfo,
                userPermissions.stream().map(String::valueOf).collect(java.util.stream.Collectors.toList()));
        // 把token放到redis中
        ...
        UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(
                sysUserDetails, user.getPassword(), userPermissions);
    ...
    public Result<String> register(HttpServletRequest request, UserForm userForm) {
        ...
        // 判断两次密码是否一致
        if (!SecretUtils.desEncrypt(userForm.getPassword())
                .equals(SecretUtils.desEncrypt(userForm.getCheckedPassword()))) {
    ...
    public Result<String> sendHeartbeat(HttpServletRequest request) {
        ...
        if (SecurityUtil.getRoleCode() == 1) {
            ...
            LocalDateTime now = utcTime.atZone(ZoneOffset.UTC).withZoneSameInstant(ZoneId.of("Asia/Shanghai"))
                    .toLocalDateTime();
            // 设置新的时间,并设置10分钟过期时间(防止用户长时间不活跃后累加异常时长)
            stringRedisTemplate.opsForValue().set(key, now.toString(), 10, TimeUnit.MINUTES);
            ...
            long seconds = durationSinceLastHeartbeat.getSeconds();

            // 防止异常累加:如果时间差超过10分钟(600秒),只记录10分钟
            // 这样可以防止用户长时间离开后重新访问时累加过多时长
            if (seconds > 600) {
                seconds = 600;
            }

            // 如果时间差小于0(时间倒退)或大于10分钟,忽略本次心跳
            if (seconds < 0) {
            ...
            if (Objects.isNull(userDailyLogin)) {
                ...
                // 存入秒数
                userDailyLoginDuration.setTotalSeconds((int) seconds);
