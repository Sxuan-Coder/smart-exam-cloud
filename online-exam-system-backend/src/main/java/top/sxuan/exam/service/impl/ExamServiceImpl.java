...
import top.sxuan.exam.converter.ExamConverter;
import top.sxuan.exam.converter.ExamQuAnswerConverter;
...
public class ExamServiceImpl extends ServiceImpl<ExamMapper, Exam> implements IExamService {
    ...
    public Result<String> createExam(ExamAddForm examAddForm) {
        ...
        if ("1".equals(examAddForm.getAddQuype())) {
            ...
            for (Map.Entry<Integer, Integer> entry : quTypeToCount.entrySet()) {
                ...
                List<Integer> typeQuestionIds = questionsByType.stream().map(Question::getId)
                        .collect(Collectors.toList());
    ...
    public Result<ExamQuestionListVO> getQuestionList(Integer examId) {
        ...
        if (!isUserTakingExam(examId)) {
            return Result.failed("未处于考试进行中状态");
        }
        ...
        if (createTime != null) {
            ...
            return Result.failed("试卷记录不存在，请重新开始考试");
        }
    ...
    public Result<Integer> addCheat(Integer examId) {
        ...
        int maxAllowCount = exam.getMaxCount() != null ? exam.getMaxCount() : 999; // 默认很大

        // 操作次数超过限制，自动交卷
        if (currentCount >= maxAllowCount) {
            this.handExam(examId);
            return Result.success("已超过最大切屏次数 (" + maxAllowCount + ")，已自动交卷", 1);
        }
        ...
        userExamsScoreLambdaUpdateWrapper.eq(UserExamsScore::getExamId, examId)
                .eq(UserExamsScore::getUserId, SecurityUtil.getUserId())
                .eq(UserExamsScore::getState, 0)
                .set(UserExamsScore::getCount, currentCount + 1);
        int insert = userExamsScoreMapper.update(null, userExamsScoreLambdaUpdateWrapper);
        return Result.success("请勿切屏，最大切屏次数：" + maxAllowCount + ",已切屏次数:" + (currentCount + 1), 0);
    }
    ...
    public Result<List<ExamRecordDetailVO>> details(Integer examId) {
        ...
        for (Question temp : questions) {
            ...
            answerWrapper.eq(ExamQuAnswer::getUserId, SecurityUtil.getUserId())
                    .eq(ExamQuAnswer::getExamId, examId)
                    .eq(ExamQuAnswer::getQuestionId, temp.getId());
            ExamQuAnswer myAnswer = examQuAnswerMapper.selectOne(answerWrapper);

            if (myAnswer != null) {
                examRecordDetailVO.setIsRight(myAnswer.getIsRight());
                examRecordDetailVO.setAiScore(myAnswer.getAiScore());
                examRecordDetailVO.setAiReason(myAnswer.getAiReason());
                examRecordDetailVO.setAiGradingDetail(myAnswer.getAiGradingDetail());

                if (temp.getQuType() == 4) {
                    examRecordDetailVO.setMyOption(myAnswer.getAnswerContent());
                } else {
                    // 处理选择题答案索引 A,B,C...
                    String answerIds = myAnswer.getAnswerId();
                    if (StringUtils.isNotBlank(answerIds)) {
                        List<Integer> myOpIds = Arrays.stream(answerIds.split(","))
                                .map(Integer::parseInt)
                                .collect(Collectors.toList());
                        List<String> mySorts = options.stream()
                                .filter(o -> myOpIds.contains(o.getId()))
                                .map(o -> String.valueOf(o.getSort()))
                                .collect(Collectors.toList());
                        examRecordDetailVO.setMyOption(String.join(",", mySorts));
                    }
                }
            }
            ...
            if (temp.getQuType() == 4 && !options.isEmpty()) {
                ...
                List<String> rightSorts = options.stream()
                        .filter(o -> o.getIsRight() == 1)
                        .map(o -> String.valueOf(o.getSort()))
                        .collect(Collectors.toList());
                examRecordDetailVO.setRightOption(String.join(",", rightSorts));
            }
    ...
    public Result<ExamQuDetailVO> handExam(Integer examId) {
        ...
        List<ExamQuestion> unansweredSaqQuestions = examQuestionMapper.getUnansweredSaqQuestions(examId,
        ...
        // 如果需要阅卷且包含简答题，触发 AI 异步自动评分
        if (whetherMark == 0) {
            autoScoringService.autoScoringExam(examId, SecurityUtil.getUserId());
            return Result.success("提交成功，AI 正在为您预评分，请稍后查看详情");
        }

        // 如果无需阅卷，检查是否需要发放证书
        if (whetherMark == -1 && examOne.getCertificateId() != null && examOne.getPassedScore() != null
                && calculatedScore >= examOne.getPassedScore()) {
    ...
    public Result<String> startExam(Integer examId) {
        ...
        userExamsScore.setCount(0); // 初始化切屏次数为0
        userExamsScore.setUserScore(0); // 初始化得分为0
    ...
    @Override
    public Result<AiComposePaperVO> aiComposePaper(AiComposePaperForm form) {
        // 1. 参数校验
        if (form.getEasyPercent() + form.getMediumPercent() + form.getHardPercent() != 100) {
            return Result.failed("难度比例之和必须为100%");
        }

        int calculatedTotalScore = form.getRadioCount() * form.getRadioScore()
                + form.getMultiCount() * form.getMultiScore()
                + form.getJudgeCount() * form.getJudgeScore()
                + form.getSaqCount() * form.getSaqScore();

        if (calculatedTotalScore > form.getTotalScore()) {
            return Result.failed("各题型得分总计超过了设定的总分上限");
        }

        // 2. 初始化预览结果
        AiComposePaperVO vo = new AiComposePaperVO();
        String previewId = UUID.randomUUID().toString();
        vo.setPreviewId(previewId);
        vo.setTitle(form.getTitle());
        vo.setTotalScore(calculatedTotalScore);
        vo.setRadioQuestions(new ArrayList<>());
        vo.setMultiQuestions(new ArrayList<>());
        vo.setJudgeQuestions(new ArrayList<>());
        vo.setSaqQuestions(new ArrayList<>());

        // 3. 处理不同类型的题目
        processLevelQuestions(form, vo);

        // 4. 存入 Redis 缓存，失效时间 30 分钟
        redisTemplate.opsForValue().set("preview:" + previewId, vo, 30, TimeUnit.MINUTES);

        return Result.success("智能组卷预览生成成功", vo);
    }

    private void processLevelQuestions(AiComposePaperForm form, AiComposePaperVO vo) {
        // 四种题型：1-单选, 2-多选, 3-判断, 4-简答
        int[] types = { 1, 2, 3, 4 };
        int[] counts = { form.getRadioCount(), form.getMultiCount(), form.getJudgeCount(), form.getSaqCount() };
        int[] scores = { form.getRadioScore(), form.getMultiScore(), form.getJudgeScore(), form.getSaqScore() };
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            int type = types[i];
            int totalNeeded = counts[i];
            int score = scores[i];

            // 简单计：根据比例分配难度
            int easyNeeded = (int) Math.round(totalNeeded * (form.getEasyPercent() / 100.0));
            int mediumNeeded = (int) Math.round(totalNeeded * (form.getMediumPercent() / 100.0));
            int hardNeeded = totalNeeded - easyNeeded - mediumNeeded;

            // 尝试从题库获取
            List<AiComposePaperVO.AiComposedQuestionVO> questions = new ArrayList<>();
            questions.addAll(fetchFromRepo(form.getRepoId(), type, 1, easyNeeded, form.getKnowledgePoints(), score));
            questions.addAll(fetchFromRepo(form.getRepoId(), type, 2, mediumNeeded, form.getKnowledgePoints(), score));
            questions.addAll(fetchFromRepo(form.getRepoId(), type, 3, hardNeeded, form.getKnowledgePoints(), score));

            // 如果题库不足，调用 AI 补充
            int currentCount = questions.size();
            if (currentCount < totalNeeded) {
                int aiNeeded = totalNeeded - currentCount;
                String prompt = String.format("题型：%d, 数量：%d, 知识点：%s", type, aiNeeded,
                        String.join(",", form.getKnowledgePoints()));
                try {
                    String aiResponse = aiChat.getChatResponse(Constants.composePaperSystemMessage + "\n" + prompt)
                            .trim();

                    // 打印日志
                    System.out.println("AI Response for type " + type + ":\n" + aiResponse);

                    String jsonPart = "";
                    int start = aiResponse.indexOf("[");
                    int end = aiResponse.lastIndexOf("]");
                    if (start != -1 && end != -1 && end > start) {
                        jsonPart = aiResponse.substring(start, end + 1).trim();
                    } else {
                        // 尝试 Markdown 提取
                    ...
                    // 预处理：移除可能存在的尾部逗号 (AI 常见错误)
                    jsonPart = jsonPart.replaceAll(",\\s*]", "]");
                    jsonPart = jsonPart.replaceAll(",\\s*\\}", "}");

                    JSONArray aiBatch = JSONUtil.parseArray(jsonPart);
                    for (int j = 0; j < aiBatch.size(); j++) {
                        JSONObject quObj = aiBatch.getJSONObject(j);
                        AiComposePaperVO.AiComposedQuestionVO aiQu = new AiComposePaperVO.AiComposedQuestionVO();
                        aiQu.setQuestionId(null);
                        aiQu.setContent(quObj.getStr("content"));
                        aiQu.setQuType(type);
                        aiQu.setQuLevel(quObj.getInt("quLevel"));
                        aiQu.setKnowledgePoint(quObj.getStr("knowledgePoint"));
                        aiQu.setAnswer(quObj.getStr("answer"));
                        aiQu.setAnalysis(quObj.getStr("analysis"));
                        aiQu.setOptions(quObj.getStr("options"));
                        aiQu.setSource("AI");
                        aiQu.setScore(score);
                        questions.add(aiQu);
                    }
                } catch (Exception e) {
                    throw new ServiceRuntimeException("AI 生成或解析题目失败: " + e.getMessage());
                }
            }

            // 分配到对应的列表
            if (type == 1)
                vo.setRadioQuestions(questions);
            else if (type == 2)
                vo.setMultiQuestions(questions);
            else if (type == 3)
                vo.setJudgeQuestions(questions);
            else if (type == 4)
                vo.setSaqQuestions(questions);
        }

    }

    private List<AiComposePaperVO.AiComposedQuestionVO> fetchFromRepo(Integer repoId, Integer type, Integer level,
            Integer count, List<String> kps, Integer score) {
        if (count <= 0)
            return new ArrayList<>();

        LambdaQueryWrapper<Question> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(Question::getRepoId, repoId)
                .eq(Question::getQuType, type)
                .eq(Question::getQuLevel, level)
                .eq(Question::getIsDeleted, 0);

        if (kps != null && !kps.isEmpty()) {
            wrapper.and(w -> {
                for (String kp : kps) {
                    w.or().like(Question::getKnowledgePoint, kp);
                }
            });
        }
        ...
        return list.stream().limit(count).map(q -> {
            AiComposePaperVO.AiComposedQuestionVO v = new AiComposePaperVO.AiComposedQuestionVO();
            v.setQuestionId(q.getId());
            v.setContent(q.getContent());
            v.setQuType(q.getQuType());
            v.setQuLevel(q.getQuLevel());
            v.setKnowledgePoint(q.getKnowledgePoint());
            v.setAnalysis(q.getAnalysis());
            v.setSource("REPO");
            v.setScore(score);
            // 这里可能需要补齐选项和答案，简单处理：
            if (q.getQuType() != 4) {
                List<Option> ops = optionMapper.selectAllByQuestionId(q.getId());
                v.setOptions(JSONUtil.toJsonStr(ops));
                Optional<Option> right = ops.stream().filter(o -> o.getIsRight() == 1).findFirst();
                v.setAnswer(right.isPresent() ? right.get().getContent() : "");
            } else {
                List<Option> ops = optionMapper.selectAllByQuestionId(q.getId());
                v.setAnswer(ops.isEmpty() ? "" : ops.get(0).getContent());
            }
            return v;
        }).collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional
    public Result<Integer> confirmAiComposePaper(String previewId, String gradeIds, Integer examDuration,
            Integer passedScore) {
        // 1. 获取预览数据
        AiComposePaperVO vo = (AiComposePaperVO) redisTemplate.opsForValue().get("preview:" + previewId);
        if (vo == null) {
            return Result.failed("预览已过期或不存在，请重新生成");
        }

        // 2. 创建试卷实体
        Exam exam = new Exam();
        exam.setTitle(vo.getTitle());
        exam.setExamDuration(examDuration);
        exam.setPassedScore(passedScore);
        exam.setGrossScore(vo.getTotalScore());
        exam.setUserId(SecurityUtil.getUserId());
        exam.setStartTime(LocalDateTime.now());
        exam.setEndTime(LocalDateTime.now().plusDays(7)); // 默认7天过期

        // 填充题型数量和分数
        exam.setRadioCount(vo.getRadioQuestions().size());
        exam.setRadioScore(vo.getRadioQuestions().isEmpty() ? 0 : vo.getRadioQuestions().get(0).getScore());
        exam.setMultiCount(vo.getMultiQuestions().size());
        exam.setMultiScore(vo.getMultiQuestions().isEmpty() ? 0 : vo.getMultiQuestions().get(0).getScore());
        exam.setJudgeCount(vo.getJudgeQuestions().size());
        exam.setJudgeScore(vo.getJudgeQuestions().isEmpty() ? 0 : vo.getJudgeQuestions().get(0).getScore());
        exam.setSaqCount(vo.getSaqQuestions().size());
        exam.setSaqScore(vo.getSaqQuestions().isEmpty() ? 0 : vo.getSaqQuestions().get(0).getScore());

        examMapper.insert(exam);

        // 3. 处理题目入库和关联
        int sort = 0;
        List<AiComposePaperVO.AiComposedQuestionVO> allQuestions = new ArrayList<>();
        allQuestions.addAll(vo.getRadioQuestions());
        allQuestions.addAll(vo.getMultiQuestions());
        allQuestions.addAll(vo.getJudgeQuestions());
        allQuestions.addAll(vo.getSaqQuestions());

        for (AiComposePaperVO.AiComposedQuestionVO quVo : allQuestions) {
            Integer questionId = quVo.getQuestionId();

            // 如果是 AI 生成的，先入库
            if ("AI".equals(quVo.getSource())) {
                Question q = new Question();
                q.setContent(quVo.getContent());
                q.setQuType(quVo.getQuType());
                q.setQuLevel(quVo.getQuLevel());
                q.setKnowledgePoint(quVo.getKnowledgePoint());
                q.setAnalysis(quVo.getAnalysis());
                q.setUserId(SecurityUtil.getUserId());
                // TODO: 需要题库ID，这里可以从Form中传过来或者暂时关联到一个特定的AI题库
                questionMapper.insert(q);
                questionId = q.getId();

                // 插入选项
                if (quVo.getQuType() != 4) {
                    JSONArray opsJson = JSONUtil.parseArray(quVo.getOptions());
                    for (int i = 0; i < opsJson.size(); i++) {
                        JSONObject opObj = opsJson.getJSONObject(i);
                        Option op = new Option();
                        op.setQuId(questionId);
                        op.setContent(opObj.getStr("content"));
                        op.setIsRight(opObj.getInt("isRight"));
                        op.setSort(i + 1);
            ...
            eq.setScore(quVo.getScore());
            eq.setType(quVo.getQuType());
            eq.setSort(sort++);
            examQuestionMapper.insert(eq);
        }

        // 4. 处理班级关联
        List<Integer> gIds = Arrays.stream(gradeIds.split(","))
                .map(Integer::parseInt)
                .collect(Collectors.toList());
        examGradeMapper.addExamGrade(exam.getId(), gIds);

        // 5. 清理缓存
        redisTemplate.delete("preview:" + previewId);

        return Result.success("试卷生成成功", exam.getId());
    }

    @Override
    public Result<List<PendingExamVO>> getPendingExams() {
        // 获取当前用户ID
        Integer userId = SecurityUtil.getUserId();
        LocalDateTime now = LocalDateTime.now();

        // 查询当前用户所在的班级
        List<Integer> gradeIds = examMapper.getUserGradeIds(userId);
        ...
        // 查询当前时间在考试范围内的考试
        LambdaQueryWrapper<Exam> examQuery = new LambdaQueryWrapper<>();
        examQuery.le(Exam::getStartTime, now) // 开始时间 <= 当前时间
                .ge(Exam::getEndTime, now) // 结束时间 >= 当前时间
                .eq(Exam::getIsDeleted, 0) // 未删除
                .orderByAsc(Exam::getStartTime); // 按开始时间升序

        List<Exam> ongoingExams = examMapper.selectList(examQuery);

        // 过滤出该用户所在班级的考试,并排除已参加的考试
        List<PendingExamVO> pendingExams = new ArrayList<>();
        for (Exam exam : ongoingExams) {
            // 检查该考试是否属于用户的班级
            List<Integer> examGrades = examMapper.getExamGradeIds(exam.getId());
            boolean isUserGrade = false;
            for (Integer gradeId : gradeIds) {
                if (examGrades.contains(gradeId)) {
                    isUserGrade = true;
                    break;
                }
            }
            ...
            scoreQuery.eq(UserExamsScore::getExamId, exam.getId())
                    .eq(UserExamsScore::getUserId, userId);
            UserExamsScore score = userExamsScoreMapper.selectOne(scoreQuery);

            // 如果已参加(有成绩记录且状态为已完成或已交卷),则跳过
            if (score != null && (score.getState() != null && score.getState() == 1)) {
                continue;
            }
        ...
        return Result.success("查询成功", pendingExams);
    }
}
