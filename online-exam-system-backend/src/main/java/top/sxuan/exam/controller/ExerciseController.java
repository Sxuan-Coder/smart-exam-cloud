...
import top.sxuan.exam.service.IExerciseRecordService;
import top.sxuan.exam.service.IRepoService;
...
public class ExerciseController {
    ...
    public Result<List<QuestionSheetVO>> getQuestion(@PathVariable("repoId") Integer repoId,
            @Min(value = 1, message = "试题类型最小值应为1") @Max(value = 4, message = "试题类型最大值应为4") @Nullable @RequestParam(value = "quType", required = false) Integer quType) {
    ...
    @PreAuthorize("hasAnyAuthority('role_student')")
    public Result<AnswerInfoVO> getAnswerInfo(@PathVariable("repoId") Integer repoId,
            @PathVariable("quId") Integer quId) {
