...
public class ExamController {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('role_teacher','role_admin')")
    public Result<String> updateExam(@Validated @RequestBody ExamUpdateForm examUpdateForm,
    ...
    @GetMapping("/paging")
    @PreAuthorize("hasAnyAuthority('role_teacher','role_admin')")
    public Result<IPage<ExamVO>> getPagingExam(
    ...
    @GetMapping("/grade")
    @PreAuthorize("hasAnyAuthority('role_teacher','role_admin','role_student')")
    public Result<IPage<ExamGradeListVO>> getGradeExamList(
            @RequestParam(value = "pageNum", required = false, defaultValue = "1") Integer pageNum,
            @RequestParam(value = "pageSize", required = false, defaultValue = "10") Integer pageSize,
            @RequestParam(value = "title", required = false) String title,
     ...
     * @return 预览结果
     */
    @ApiOperation("AI 自动组卷预览")
    @PostMapping("/ai-compose")
    @PreAuthorize("hasAnyAuthority('role_teacher','role_admin')")
    public Result<AiComposePaperVO> aiComposePaper(@Validated @RequestBody AiComposePaperForm form) {
        return examService.aiComposePaper(form);
    }
     ...
     * @return 试卷ID
     */
    @ApiOperation("确认 AI 组卷提示")
    @PostMapping("/ai-compose/confirm")
    @PreAuthorize("hasAnyAuthority('role_teacher','role_admin')")
    public Result<Integer> confirmAiComposePaper(@RequestParam("previewId") String previewId,
            @RequestParam("gradeIds") String gradeIds,
            @RequestParam("examDuration") Integer examDuration,
            @RequestParam("passedScore") Integer passedScore) {
        return examService.confirmAiComposePaper(previewId, gradeIds, examDuration, passedScore);
    }
