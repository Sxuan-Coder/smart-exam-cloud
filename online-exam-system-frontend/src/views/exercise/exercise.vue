  ...
  methods: {
    ...
    initQuId() {
      ...
      if (this.paperData.radioList.length > 0) {
        ...
        this.curQuId = this.paperData.multiList[0].quId
        this.curListIndex = 2
      } else if (this.paperData.judgeList.length > 0) {
        this.curQuId = this.paperData.judgeList[0].quId
        this.curListIndex = 3
      } else if (this.paperData.saqList.length > 0) {
        this.curQuId = this.paperData.saqList[0].quId
        this.curListIndex = 4
      } else {
        // 如果所有题型都为空,提示用户
        this.$message.warning('该题库暂无试题')
