<template>
  <div class="app-container">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-plus" @click="screenInfo()">新增</el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="success" icon="el-icon-upload2" @click="fileDialogVisible = true">导入</el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="warning" icon="el-icon-cpu" @click="openAiDialog">AI 出题</el-button>
      </el-form-item>
    </el-form>
      ...
      <el-form :model="aiForm" label-width="100px">
        <el-form-item label="参考文本">
          <el-input
            ...
            type="textarea"
            :rows="6"
            placeholder="请输入学习资料、大纲或知识点描述，AI 将据此出题..."
          />
        </el-form-item>
        <el-row>
          <el-col :span="12">
            <el-form-item label="题目类型">
              <el-select v-model="aiForm.quType" placeholder="请选择">
        ...
        <el-form-item label="目标题库">
          <repo-select v-model="aiForm.repoId" />
        </el-form-item>
      </el-form>

      <!-- 预览生成结果 -->
      <div v-if="aiPreviewList.length > 0" class="ai-preview-container">
        <div class="preview-title">生成结果预览</div>
        <el-card v-for="(q, index) in aiPreviewList" :key="index" class="preview-card">
          <div slot="header">
            <span>第 {{ index + 1 }} 题 - {{ q.knowledgePoint || '未识别知识点' }}</span>
            <el-tag size="mini" :type="q.quLevel === 3 ? 'danger' : (q.quLevel === 2 ? 'warning' : 'success')" style="margin-left: 10px">
              {{ q.quLevel === 3 ? '困难' : (q.quLevel === 2 ? '中等' : '简单') }}
            </el-tag>
          </div>
          <div class="q-content">{{ q.content }}</div>
          <div v-if="q.quType !== 4" class="q-options">
            <div v-for="(opt, oIdx) in q.options" :key="oIdx" :class="{'is-right': opt.isRight}">
              {{ String.fromCharCode(65 + oIdx) }}. {{ opt.content }}
            </div>
          </div>
          <div class="q-analysis"><strong>解析：</strong>{{ q.analysis }}</div>
        </el-card>
      </div>

      <div slot="footer" class="dialog-footer">
        <el-button @click="aiDialogVisible = false">取消</el-button>
        <el-button type="warning" :loading="aiLoading" @click="handleAiGenerate">{{ aiPreviewList.length > 0 ? '重新生成' : '开始出题' }}</el-button>
        <el-button v-if="aiPreviewList.length > 0" type="primary" @click="handleAiSave">确认并导入</el-button>
      </div>
    </el-dialog>
    ...
    >
      ...
      <el-table-column label="难度" align="center" width="100">
        <template slot-scope="scope">
          <el-tag size="mini" :type="scope.row.quLevel === 3 ? 'danger' : (scope.row.quLevel === 2 ? 'warning' : 'success')">
             {{ scope.row.quLevel === 3 ? '困难' : (scope.row.quLevel === 2 ? '中等' : '简单') }}
          </el-tag>
        </template>
      </el-table-column>
...
<script>
import { quPaging, quDel, quUpdate, importQue, quAiGenerate, quBatchAdd } from '@/api/question'
...
export default {
  ...
  methods: {
    ...
    async handleAiGenerate() {
      if (!this.aiForm.text) {
        return this.$message.warning('请输入点参考文本吧，老板')
      }
      this.aiLoading = true
      try {
        const res = await quAiGenerate(this.aiForm)
        this.aiPreviewList = res.data
        this.$message.success('生成完毕，请检阅！')
      } catch (e) {
        console.error(e)
      } finally {
    ...
    async handleAiSave() {
      if (!this.aiForm.repoId) {
        return this.$message.warning('请先选择一个目标题库')
      }
      try {
        const list = this.aiPreviewList.map(q => ({
          ...q,
          repoId: this.aiForm.repoId
        }))
        const res = await quBatchAdd(list)
        if (res.code) {
          this.$message.success('已全部录入题库！')
